...
const prefixPath = path => {
    if (_helper2.default.isString(path) && !path.startsWith('/')) {
        path = '/' + path;
    }
    return path;
};
...
const matchRoute = (path, route) => {
    ...
    if (_helper2.default.isString(route)) {
        route = route.split('/');
        path = path.split(/\/+/);
...
function invokeResource(options) {
    ...
    (0, _assert2.default)(_helper2.default.isRegExp(publicPath) || _helper2.default.isString(publicPath), 'publicPath must be regexp or string');
