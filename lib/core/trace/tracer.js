...
module.exports = class Tracer {
    ...
    run(ctx, err) {
        this.ctx = ctx;
        ctx.type = _.isFunction(this.contentType) ? this.contentType(ctx) : 'html';

        const isJson = _.isFunction(ctx.response.is) && ctx.response.is('json');
        const isJsonp = _.isFunction(ctx.isJsonp) && ctx.isJsonp();
        if (isJson || isJsonp) {
            if (!_.isFunction(ctx.json)) {
                ctx.json = res => {
