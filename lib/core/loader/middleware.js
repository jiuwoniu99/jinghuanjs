...
class Middleware {
     ...
     */
    createRegexp(match) {
        if (_.isFunction(match)) return match;
     ...
     */
    checkMatch(rule, ctx) {
        if (_.isFunction(rule)) return rule(ctx);
    ...
    parse(middlewares = [], middlewarePkg = {}, app) {
        return middlewares.map(item => {
            ...
            if (_.isString(item.handle)) {
                item.handle = middlewarePkg[item.handle];
            }
            assert(_.isFunction(item.handle), 'handle must be a function');
            ...
            if (_.isFunction(options)) {
                ...
                assert(_.isFunction(item.handle), 'handle must return a function');
            }
            return item;
        }).map(item => {
