...
class Middleware {
   ...
   * },
	 ...
	 */
	loadFiles(appPath, isMultiModule, jinghuanPath) {
		const sysMiddlewares = this.getFiles(path.join(jinghuanPath, 'lib/middleware'));
	...
	load(appPath, jinghuanPath, modules, app) {
		let filepath = '';
		//if (modules.length) {
		filepath = path.join(jinghuan.ROOT_PATH, 'common/config/middleware.js');
		...
		return this.parse(middlewares, this.loadFiles(appPath, modules.length, jinghuanPath), app);
	}
}
