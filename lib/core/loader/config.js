...
class Config {
     ...
     */
     formatAdapter(config, appAdapters) {
        for (const name in config) {
            assert(_.isObject(config[name]), `adapter.${name} must be an object`);
            ...
            assert(_.isObject(common), `adapter.${name}.common must be an object`);
            ...
            for (const type in config[name]) {
                ...
                if (item.handle && _.isString(item.handle)) {
    ...
    load(appPath, jinghuanPath, env, modules) {
        const jinghuanConfig = this.loadConfigFile(path.join(jinghuanPath, 'lib', 'config'));
        const commonConfig = this.loadConfigFile(path.join(jinghuan.ROOT_PATH, 'common', 'config'));
        const envConfig = this.loadConfigFile(path.join(jinghuan.ROOT_PATH, 'config', env));
        const paths = [
            path.join(jinghuanPath, 'lib/bootstrap'),
            path.join(jinghuan.ROOT_PATH, 'common/bootstrap')
        ];
        const config = this.loadConfig(paths, env);
        const result = {};
        result['common'] = helper.extend({}, config, jinghuanConfig, commonConfig, envConfig, true);
        return result;
    }
